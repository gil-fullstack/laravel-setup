...
@section('content')
      ...
      <div class="carousel-inner h-100">
        <div class="carousel-item active h-100" style="background-image:url({{asset('storage/'.$banner_pagina->valor)}}); background-size:cover; background-position:center;">
          ...
          <div class="container h-100 position-relative d-flex justify-content-center align-items-center flex-column">
            <h1 class="mt-5 mb-0">Checkout</h1>
    ...
    <div class="container py-4">
      <form method="POST" action="{{url('checkout')}}" id="form_checkout">
        @csrf
        <div class="row">
                ...
                <div class="my-4">
                  ...
                  @foreach ($produto->precos->sortBy('categoria_id')->values() as $key => $item)
                    <div class="form-check">
                      <input class="form-check-input" type="radio" name="produto" id="{{'exampleRadios'.$key}}" value="{{$item->id}}" {{app('request')->input('plano') == $item->categoria_id ? 'checked' : ((!app('request')->input('plano') && $key == 1) ? 'checked' : '')}}>
          ...
          <div class="col-md-8 px-2">
            <div class="card p-3">
                ...
                <button type="button" id="submit_form_checkout" class="btn btn-primary" style="background-color:#0B2C52; border-color:#0B2C52">Finalizar e escolher forma de pagamento</button>
