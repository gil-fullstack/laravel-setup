...
bt.widget.prototype.init = function(jsConfig, type) {
    ...
    clientConfig = $.extend(!0, {}, this.getHTMLConfig() || {});
    $.extend(!0, clientConfig, jsConfig || {});
...
bt.widget.prototype.getConfig = function(clientConfig, type) {
    ...
    config = $.extend(!0, {}, bt[type].config.base);
    uiLibrary = clientConfig.hasOwnProperty('uiLibrary') ? clientConfig.uiLibrary : config.uiLibrary;
    if (bt[type].config[uiLibrary]) {
        $.extend(!0, config, bt[type].config[uiLibrary])
    }
    iconsLibrary = clientConfig.hasOwnProperty('iconsLibrary') ? clientConfig.iconsLibrary : config.iconsLibrary;
    if (bt[type].config[iconsLibrary]) {
        $.extend(!0, config, bt[type].config[iconsLibrary])
    }
    for (plugin in bt[type].plugins) {
        if (bt[type].plugins.hasOwnProperty(plugin)) {
            $.extend(!0, config, bt[type].plugins[plugin].config.base);
            if (bt[type].plugins[plugin].config[uiLibrary]) {
                $.extend(!0, config, bt[type].plugins[plugin].config[uiLibrary])
            }
            if (bt[type].plugins[plugin].config[iconsLibrary]) {
                $.extend(!0, config, bt[type].plugins[plugin].config[iconsLibrary])
            }
        }
    }
    $.extend(!0, config, clientConfig);
    if (!config.guid) {
...
bt.widget.prototype.reload = function(params) {
    ...
    if ($.isArray(data.dataSource)) {
        ...
        ajaxOptions = $.extend(!0, {}, data.dataSource);
...
bt.core = {
    ...
    formatDate: function(date, format, locale) {
        var result = '',
            separator, tmp, formatParts = format.split(/[\s,-\.//\:]+/),
    ...
    setChildPosition: function(mainEl, childEl) {
        var mainElRect = mainEl.getBoundingClientRect(),
            mainElHeight = bt.core.height(mainEl, !0),
            childElHeight = bt.core.height(childEl, !0),
            mainElWidth = bt.core.width(mainEl, !0),
            childElWidth = bt.core.width(childEl, !0),
    ...
    position: function(elem, padding, margin) {
        ...
        return {
            top: top,
            left: left,
            bottom: top + bt.core.height(elem),
            right: left + bt.core.width(elem)
        }
    }
};
bt.dialog = {
...
bt.dialog.methods = {
    ...
    renderHeader: function($dialog) {
        ...
        } else if ($closeButton.length > 0 && data.closeButtonInHeader === !1) {
            $closeButton.hide()
        } else {
            $closeButton.addClass(data.style.headerCloseButton)
        }
    ...
    resizable: function($dialog) {
        ...
        $dialog.append($('<div class="bt-resizable-handle bt-resizable-e"></div>').draggable($.extend(!0, {
            vertical: !1
        }, config)));
        $dialog.append($('<div class="bt-resizable-handle bt-resizable-s"></div>').draggable($.extend(!0, {
            horizontal: !1
        }, config)));
        $dialog.append($('<div class="bt-resizable-handle bt-resizable-w"></div>').draggable($.extend(!0, {
            vertical: !1
        }, config)));
        $dialog.append($('<div class="bt-resizable-handle bt-resizable-ne"></div>').draggable($.extend(!0, {}, config)));
        $dialog.append($('<div class="bt-resizable-handle bt-resizable-nw"></div>').draggable($.extend(!0, {}, config)));
        $dialog.append($('<div class="bt-resizable-handle bt-resizable-sw"></div>').draggable($.extend(!0, {}, config)));
        $dialog.append($('<div class="bt-resizable-handle bt-resizable-se"></div>').draggable($.extend(!0, {}, config)))
    },
    resize: function(e, newPosition) {
        var $el, $dialog, position, data, height, width, top, left, result = !1;
        $el = $(this);
        $dialog = $el.parent();
...
bt.draggable.methods = {
    init: function(jsConfig) {
        ...
        $handleEl.on('touchstart mousedown', function(e) {
            ...
            $dragEl.attr('draggable-dragging', !0);
    ...
    createUpHandler: function($dragEl) {
        return function(e) {
            if ($dragEl.attr('draggable-dragging') === 'true') {
                $dragEl.attr('draggable-dragging', !1);
                ...
                bt.draggable.events.stop($dragEl, {
                    x: $dragEl.mouseX(e),
                    y: $dragEl.mouseY(e)
                })
            }
    ...
    move: function(dragEl, data, offsetX, offsetY, mouseX, mouseY) {
        var contPosition, maxTop, maxLeft, position = bt.core.position(dragEl),
        ...
        if (!1 !== bt.draggable.events.drag($(dragEl), newTop, newLeft, mouseX, mouseY)) {
...
bt.droppable.methods = {
    ...
    isOver: function($dropEl, mousePosition) {
        ...
        return mousePosition.x > offsetLeft && mousePosition.x < (offsetLeft + $dropEl.outerWidth(!0)) && mousePosition.y > offsetTop && mousePosition.y < (offsetTop + $dropEl.outerHeight(!0))
    },
...
bt.grid.methods = {
    ...
    setDefaultColumnConfig: function(columns, defaultColumnSettings) {
        ...
        if (columns && columns.length) {
            for (i = 0; i < columns.length; i++) {
                column = $.extend(!0, {}, defaultColumnSettings);
                $.extend(!0, column, columns[i]);
                columns[i] = column
            }
        }
    },
    ...
    updateHeader: function($grid) {
        var $sortIcon, $cellTitle, data = $grid.data(),
    ...
    useHtmlDataSource: function($grid, data) {
        var dataSource = [],
            i, j, $cells, record, $rows = $grid.find('tbody tr[data-role != "empty"]');
    ...
    startLoading: function($grid) {
        ...
        width = $tbody.outerWidth(!1);
        height = $tbody.outerHeight(!1);
        ...
        $loading.css({
            top: top + (height / 2) - ($loading.outerHeight(!1) / 2),
            left: (width / 2) - ($loading.outerWidth(!1) / 2)
        })
    },
    ...
    loadData: function($grid) {
        ...
        if ('checkbox' === data.selectionMethod && 'multiple' === data.selectionType) {
            $grid.find('thead input[data-role="selectAll"]').prop('checked', !1)
        }
            ...
            for (key in column.events) {
                if (column.events.hasOwnProperty(key)) {
                    $cell.on(key, {
                        ...
                        field: column.field,
                        record: record
                    }, bt.grid.methods.createCellEventHandler(column, column.events[key]))
                }
            }
        }
        if (column.hidden) {
    ...
    renderDisplayElement: function($grid, $displayEl, column, record, id, mode) {
        ...
        if ('checkbox' === column.type && bt.checkbox) {
            if ('create' === mode) {
                $checkbox = $('<input type="checkbox" />').val(id).prop('checked', (record[column.field] ? !0 : !1));
                ...
                $displayEl.find('input[type="checkbox"]').val(id).prop('checked', (record[column.field] ? !0 : !1))
            }
    ...
    selectRow: function($grid, data, $row, id) {
        ...
        if ('checkbox' === data.selectionMethod) {
            ...
            $checkbox.length && !$checkbox.prop('checked') && $checkbox.prop('checked', !0);
            if ('multiple' === data.selectionType && $grid.getSelections().length === $grid.count(!1)) {
                $grid.find('thead input[data-role="selectAll"]').prop('checked', !0)
            }
        }
    ...
    unselectRow: function($grid, data, $row, id) {
        ...
        if ($row.attr('data-selected') === 'true') {
            ...
            if ('checkbox' === data.selectionMethod) {
                ...
                $checkbox.length && $checkbox.prop('checked') && $checkbox.prop('checked', !1);
    ...
    unSelectAll: function($grid) {
        ...
        $grid.find('tbody tr').each(function() {
            var $row = $(this),
                ...
                id = bt.grid.methods.getId(record, data.primaryKey, position);
            bt.grid.methods.unselectRow($grid, data, $row, id);
            $row.find('input[type="checkbox"][data-role="selectRow"]').prop('checked', !1)
        });
        $grid.find('thead input[data-role="selectAll"]').prop('checked', !1);
        return $grid
    },
    ...
    getSelections: function($grid) {
        var result = [],
            position, record, data = $grid.data(),
    ...
    getByPosition: function($grid, position) {
        return $grid.getAll(!1)[position - 1]
    },
    ...
    filter: function($grid) {
        var field, column, data = $grid.data(),
    ...
    destroy: function($grid, keepTableTag, keepWrapperTag) {
        ...
        if (data) {
            ...
            $grid.xhr && $grid.xhr.abort();
            $grid.off();
            if (keepWrapperTag === !1 && $grid.parent('div[data-role="wrapper"]').length > 0) {
                $grid.unwrap()
            }
            $grid.removeData();
            if (keepTableTag === !1) {
    ...
    showColumn: function($grid, field) {
        ...
        if (position > -1) {
            ...
            data.columns[position].hidden = !1;
    ...
    hideColumn: function($grid, field) {
        ...
        if (position > -1) {
            ...
            data.columns[position].hidden = !0;
            $cells = $grid.find('tbody > tr[data-role="empty"] > td');
            if ($cells && $cells.length) {
                $cells.attr('colspan', bt.grid.methods.countVisibleColumns($grid))
            }
...
bt.grid.plugins.fixedHeader = {
    ...
    configure: function($grid, fullConfig, clientConfig) {
        $.extend(!0, $grid, bt.grid.plugins.fixedHeader.public);
        var data = $grid.data();
...
bt.grid.plugins.expandCollapseRows = {
    ...
    'configure': function($grid) {
        ...
        $.extend(!0, $grid, bt.grid.plugins.expandCollapseRows.public);
        if (typeof(data.detailTemplate) !== 'undefined') {
            column = {
                title: '',
                ...
                events: {
                    'click': function(e) {
                        var $cell = $(this),
                            methods = bt.grid.plugins.expandCollapseRows.private;
    ...
    base: {
        ...
        inlineEditing: {
            ...
            managementColumnConfig: {
                ...
                align: 'center',
                renderer: bt.grid.plugins.inlineEditing.renderers.editManager,
                cssClass: 'bt-grid-management-column'
            }
        }
    },
    bootstrap: {
    ...
    displayMode: function($grid, $cell, column, cancel) {
        ...
        if (column.mode !== 'editOnly') {
            if ($cell.attr('data-mode') === 'edit') {
                ...
                record = $grid.get(position);
                if (cancel !== !0 && newValue !== record[column.field]) {
...
bt.grid.plugins.inlineEditing.public = {
    ...
    update: function(id) {
        var i, record = this.getById(id),
            ...
            columns = this.data('columns');
        for (i = 0; i < $cells.length; i++) {
            bt.grid.plugins.inlineEditing.private.displayMode(this, $($cells[i]), columns[i], !1)
        }
    ...
    cancel: function(id) {
        var i, record = this.getById(id),
            ...
            columns = this.data('columns');
        for (i = 0; i < $cells.length; i++) {
            bt.grid.plugins.inlineEditing.private.displayMode(this, $($cells[i]), columns[i], !0)
        }
        return this
    }
};
...
bt.grid.plugins.inlineEditing.configure = function($grid, fullConfig, clientConfig) {
    var data = $grid.data();
    $.extend(!0, $grid, bt.grid.plugins.inlineEditing.public);
    ...
    private: {
        ...
        saveParams: function($grid) {
            ...
            if (data.optimisticPersistence.sessionStorage) {
                ...
                storage = $.extend(!0, JSON.parse(sessionStorage.getItem('bt.grid.' + data.guid)), storage);
            ...
            if (data.optimisticPersistence.localStorage) {
                ...
                storage = $.extend(!0, JSON.parse(localStorage.getItem('bt.grid.' + data.guid)), storage);
...
bt.grid.plugins.pagination = {
    ...
    private: {
        init: function($grid) {
            ...
            if (data.pager) {
                ...
                leftControls = bt.grid.methods.clone(data.pager.leftControls);
                $.each(leftControls, function() {
                    $leftPanel.append(this)
                });
                rightControls = bt.grid.methods.clone(data.pager.rightControls);
        ...
        assignPageHandler: function($grid, $control, newPage, disabled) {
            ...
            if (disabled) {
                $control.addClass(style.stateDisabled).prop('disabled', !0).off('click')
            } else {
                $control.removeClass(style.stateDisabled).prop('disabled', !1).off('click').on('click', function() {
        ...
        changePage: function($grid, newPage) {
            var data = $grid.data();
            if (bt.grid.plugins.pagination.events.pageChanging($grid, newPage) !== !1 && !isNaN(newPage)) {
    ...
    configure: function($grid, fullConfig, clientConfig) {
        $.extend(!0, $grid, bt.grid.plugins.pagination.public);
        var data = $grid.data();
...
bt.grid.plugins.responsiveDesign = {
    ...
    'configure': function($grid, fullConfig, clientConfig) {
        $.extend(!0, $grid, bt.grid.plugins.responsiveDesign.public);
...
bt.grid.plugins.toolbar = {
    ...
    configure: function($grid) {
        $.extend(!0, $grid, bt.grid.plugins.toolbar.public);
        $grid.on('initialized', function() {
...
bt.grid.plugins.resizableColumns = {
    ...
    public: {},
    configure: function($grid, fullConfig, clientConfig) {
        $.extend(!0, $grid, bt.grid.plugins.resizableColumns.public);
...
bt.grid.plugins.rowReorder = {
    ...
    private: {
        init: function($grid) {
            var i, columnPosition, $row, $rows = $grid.find('tbody tr[data-role="row"]');
    ...
    public: {},
    configure: function($grid, fullConfig, clientConfig) {
        $.extend(!0, $grid, bt.grid.plugins.rowReorder.public);
...
bt.grid.plugins.export = {
    ...
    configure: function($grid) {
        $.extend(!0, $grid, bt.grid.plugins.export.public)
    }
};
bt.grid.plugins.columnReorder = {
    ...
    public: {},
    configure: function($grid, fullConfig, clientConfig) {
        $.extend(!0, $grid, bt.grid.plugins.columnReorder.public);
...
bt.grid.plugins.headerFilter = {
    ...
    events: {},
    configure: function($grid, fullConfig, clientConfig) {
        $.extend(!0, $grid, bt.grid.plugins.headerFilter.public);
        var data = $grid.data();
...
bt.grid.plugins.grouping = {
    ...
    configure: function($grid) {
        var column, data = $grid.data();
        $.extend(!0, $grid, bt.grid.plugins.grouping.public);
...
bt.tree.methods = {
    ...
    loadData: function($tree) {
        var i, records = $tree.data('records'),
    ...
    appendNode: function($tree, $parent, nodeData, level, position) {
        ...
        var i, $node, $newParent, $span, $img, data = $tree.data(),
        ...
        // lista_caminho_arvore.push(obj_json_node);
        // console.log(nodeData.data);    
            if(nodeData.data.checked){

                $display = $('<span data-role="display" class="selecionado">' + nodeData.data[data.textField] + '</span>');

            }else{

                $display = $('<span data-role="display" class="nao_selecionado">' + nodeData.data[data.textField] + '</span>');

            }

            var hasChildren = typeof(nodeData.data[data.hasChildrenField]) !== 'undefined' && nodeData.data[data.hasChildrenField].toString().toLowerCase() === 'true',
    ...
    expand: function($tree, $node, cascade) {
        var $children, i, $expander = $node.find('>[data-role="wrapper"]>[data-role="expander"]'),
            data = $tree.data(),
            id = $node.attr('data-id'),
            $list = $node.children('ul');
        if (bt.tree.events.expand($tree, $node, id) !== !1 && $list && $list.length) {
            $list.show();
    ...
    collapse: function($tree, $node, cascade) {
        var $children, i, $expander = $node.find('>[data-role="wrapper"]>[data-role="expander"]'),
            ...
            id = $node.attr('data-id'),
            $list = $node.children('ul');
        if (bt.tree.events.collapse($tree, $node, id) !== !1 && $list && $list.length) {
            $list.hide();
    ...
    expandAll: function($tree) {
        ...
        for (i = 0; i < $nodes.length; i++) {
            bt.tree.methods.expand($tree, $($nodes[i]), !0)
        }
        return $tree
    },
    collapseAll: function($tree) {
        var i, $nodes = $tree.find('ul>li');
        for (i = 0; i < $nodes.length; i++) {
            bt.tree.methods.collapse($tree, $($nodes[i]), !0)
        }
        return $tree
    },
    ...
    selectAll: function($tree) {
        var i, $nodes = $tree.find('ul>li');
        for (i = 0; i < $nodes.length; i++) {
            bt.tree.methods.select($tree, $($nodes[i]), !0)
        }
        return $tree
    },
    select: function($tree, $node, cascade) {
        var i, $children, data = $tree.data();
        if ($node.attr('data-selected') !== 'true' && bt.tree.events.select($tree, $node, $node.attr('data-id')) !== !1) {
    ...
    unselectAll: function($tree) {
        var i, $nodes = $tree.find('ul>li');
        for (i = 0; i < $nodes.length; i++) {
            bt.tree.methods.unselect($tree, $($nodes[i]), !0)
        }
        return $tree
    },
    unselect: function($tree, $node, cascade) {
        var i, $children, data = $tree.data();
        if ($node.attr('data-selected') === 'true' && bt.tree.events.unselect($tree, $node, $node.attr('data-id')) !== !1) {
    ...
    getAll: function($tree, records) {
        var i, $node, id, targetRecord, result = [],
    ...
    addNode: function($tree, data, $parent, position) {
        var level, newNodeData = bt.tree.methods.getRecords($tree, [data])[0];
    ...
    getChildren: function($tree, $node, cascade) {
        var result = [],
            i, $children, cascade = typeof(cascade) === 'undefined' ? !0 : cascade;
        if (cascade) {
            $children = $node.find('ul li')
        } else {
            $children = $node.find('>ul>li')
    ...
    enableAll: function($tree) {
        ...
        for (i = 0; i < $children.length; i++) {
            bt.tree.methods.enableNode($tree, $($children[i]), !0)
        }
        return $tree
    },
    enableNode: function($tree, $node, cascade) {
        var i, $children, $expander = $node.find('>[data-role="wrapper"]>[data-role="expander"]'),
            $display = $node.find('>[data-role="wrapper"]>[data-role="display"]'),
            cascade = typeof(cascade) === 'undefined' ? !0 : cascade;
    ...
    disableAll: function($tree) {
        var i, $children = $tree.find('ul>li');
        for (i = 0; i < $children.length; i++) {
            bt.tree.methods.disableNode($tree, $($children[i]), !0)
        }
        return $tree
    },
...
bt.tree.plugins.checkboxes = {
    ...
    private: {
        ...
        nodeDataBound: function($tree, $node, id, record) {
            ...
            disabled && $checkbox.prop('disabled', !0);
    ...
    events: {
        checkboxChange: function($tree, $node, record, state) {
            ...
            if(record.checked){
                ...
                inputs_selecionados.pop(record);
            
            }else{

                record.checked = true;
                inputs_selecionados.push(record);

            }
            // console.log(inputs_selecionados);
            return $tree.triggerHandler('checkboxChange', [$node, record, state])
        }
    },
    configure: function($tree) {
        if ($tree.data('checkboxes') && bt.checkbox) {
            $.extend(!0, $tree, bt.tree.plugins.checkboxes.public);
            ...
            $tree.on('enable', function(e, $node) {
                ...
                $node.find('>[data-role="wrapper"]>[data-role="checkbox"] input[type="checkbox"]').prop('disabled', !1)
            });
            $tree.on('disable', function(e, $node) {
                $node.find('>[data-role="wrapper"]>[data-role="checkbox"] input[type="checkbox"]').prop('disabled', !0)
            })
        }
    }
};
bt.tree.plugins.dragAndDrop = {
    ...
    private: {
        ...
        createNodeMouseDownHandler: function($tree) {
            return function(e) {
                $tree.data('dragReady', !0)
            }
        },
        createNodeMouseUpHandler: function($tree) {
            return function(e) {
                $tree.data('dragReady', !1)
            }
        },
        createNodeMouseMoveHandler: function($tree, $node, $display) {
            return function(e) {
                if ($tree.data('dragReady')) {
                    ...
                    offsetLeft -= $dragEl.find('[data-role="indicator"]').outerWidth(!0);
                    $dragEl.css({
                        position: 'absolute',
                        top: offsetTop,
                        left: offsetLeft,
                        width: $display.outerWidth(!0)
                    });
            ...
            return function(e, mousePosition) {
                ...
                $displays.each(function() {
                    ...
                    if ($targetDisplay.droppable('isOver', mousePosition)) {
                        ...
                        if (bt.tree.plugins.dragAndDrop.events.nodeDrop($tree, $sourceNode.data('id'), $targetNode.data('id'), $ul.children('li').length + 1) !== !1) {
                ...
                if (!success) {
                    $wrappers.each(function() {
                        ...
                        if ($targetWrapper.droppable('isOver', mousePosition)) {
                            ...
                            if (bt.tree.plugins.dragAndDrop.events.nodeDrop($tree, sourceNodeId, $targetNode.parent('ul').parent('li').data('id'), orderNumber) !== !1) {
                                if (prepend) {
    ...
    configure: function($tree) {
        $.extend(!0, $tree, bt.tree.plugins.dragAndDrop.public);
        ...
        createDoneHandler: function($tree, $node) {
            return function(response) {
                ...
                if (response && response.length) {
                    ...
                    $expander = $node.find('>[data-role="wrapper"]>[data-role="expander"]'), $expander.attr('data-mode', 'open');
                    $expander.empty().append(data.icons.collapse);
                    bt.tree.events.dataBound($tree)
                }
            }
...
bt.checkbox.config = {
    ...
    fontawesome: {
        style: {
            iconsCssClass: 'bt-checkbox-bootstrap',
            // spanCssClass: 'fa'
        }
    }
};
...
bt.editor.methods = {
    ...
    initialize: function($editor) {
        ...
        $body.attr('contenteditable', !0);
        $body.on('keydown', function(e) {
            var key = event.keyCode || event.charCode;
            if (bt.editor.events.changing($editor) === !1 && key !== 8 && key !== 46) {
                e.preventDefault()
            }
        });
        ...
        $body.height(data.height - bt.core.height($toolbar[0], !0))
    },
    localization: function(data) {
    ...
    executeCmd: function($editor, $body, $toolbar, $btn) {
        $body.focus();
        document.execCommand($btn.attr('role'), !1);
...
bt.dropdown.methods = {
    ...
    initialize: function($dropdown) {
        ...
        if ($wrapper.length === 0) {
            $wrapper = $('<div role="wrapper" />').addClass(data.style.wrapper);
    ...
    setListPosition: function(presenter, list, data) {
        var top, listHeight, presenterHeight, newHeight, listElRect, mainElRect = presenter.getBoundingClientRect(),
            scrollY = window.scrollY || window.pageYOffset || 0,
            scrollX = window.scrollX || window.pageXOffset || 0;
        list.style.overflow = '';
        ...
        listHeight = bt.core.height(list, !0);
        listElRect = list.getBoundingClientRect();
        presenterHeight = bt.core.height(presenter, !0);
    ...
    useHtmlDataSource: function($dropdown, data) {
        var dataSource = [],
            i, record, $options = $dropdown.find('option');
...
bt.datepicker.methods = {
    ...
    initialize: function($datepicker, data) {
        var $calendar, $rightIcon, $wrapper = $datepicker.parent('div[role="wrapper"]');
        if (data.uiLibrary === 'bootstrap') {
    ...
    createCalendar: function($datepicker, data) {
        var date, $body, $footer, $btnCancel, $btnOk, $calendar = $('<div role="calendar" type="month"/>').addClass(data.style.calendar).attr('guid', $datepicker.attr('data-guid'));
    ...
    renderMonth: function($datepicker, $calendar, data) {
        var weekDay, selectedDay, day, month, year, daysInMonth, total, firstDayPosition, i, now, prevMonth, nextMonth, $cell, $day, date, $body = $calendar.children('[role="body"]'),
            $table = $('<table/>'),
            $tbody = $('<tbody/>');
        $body.off().empty();
    ...
    renderYear: function($datepicker, $calendar, data) {
        var year, i, m, $month, $table = $calendar.find('>[role="body"]>table'),
    ...
    renderDecade: function($datepicker, $calendar, data) {
        var year, decade, i, y, $year, $table = $calendar.find('>[role="body"]>table'),
            $tbody = $table.children('tbody');
        $table.children('thead').hide();
        year = parseInt($calendar.attr('year'), 10);
        decade = year - (year % 10);
    ...
    renderCentury: function($datepicker, $calendar, data) {
        var year, century, i, d, $decade, $table = $calendar.find('>[role="body"]>table'),
            $tbody = $table.children('tbody');
        $table.children('thead').hide();
        year = parseInt($calendar.attr('year'), 10);
    ...
    prev: function($datepicker, data) {
        return function() {
            var date, month, year, decade, century, $calendar = $('body').find('[role="calendar"][guid="' + $datepicker.attr('data-guid') + '"]');
    ...
    dayClickHandler: function($datepicker, $calendar, data, date) {
        return function(e) {
            ...
            if (data.footer !== !0 && data.autoClose !== !1) {
                bt.datepicker.methods.change($datepicker, $calendar, data, date)
            }
            return $datepicker
        }
...
bt.timepicker.methods = {
    ...
    updateArrow: function(e, $timepicker, $clock, data) {
        var rect, value, mouseX = $timepicker.mouseX(e),
...
bt.datetimepicker.methods = {
    ...
    getConfig: function(clientConfig, type) {
        ...
        if (bt.datepicker.config[uiLibrary]) {
            $.extend(!0, config.datepicker, bt.datepicker.config[uiLibrary])
        }
        if (bt.timepicker.config[uiLibrary]) {
            $.extend(!0, config.timepicker, bt.timepicker.config[uiLibrary])
        }
        iconsLibrary = clientConfig.hasOwnProperty('iconsLibrary') ? clientConfig.iconsLibrary : config.iconsLibrary;
        if (bt.datepicker.config[iconsLibrary]) {
            $.extend(!0, config.datepicker, bt.datepicker.config[iconsLibrary])
        }
        if (bt.timepicker.config[iconsLibrary]) {
            $.extend(!0, config.timepicker, bt.timepicker.config[iconsLibrary])
        }
        return config
    },
    initialize: function($datetimepicker) {
        var $picker, $header, $date, $time, date, $switch, $calendarMode, $clockMode, data = $datetimepicker.data();
        ...
        $datetimepicker.on('select', function(e, type) {
            ...
            if (type === 'day') {
                ...
                if ($picker.attr('selectedDay') && data.footer !== !0) {
...
bt.slider.methods = {
    ...
    createMouseMoveHandler: function($slider, track, handle, progress, data) {
        return function(e) {
            ...
            if (handle.getAttribute('drag') === 'true') {
                sliderPos = bt.core.position($slider[0], !0, !0);
